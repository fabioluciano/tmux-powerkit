...
plugin_setup_keybindings() {
    ...
    # Use pk_bind_shell directly (consistent with other plugins: bitwarden, terraform, k8s, etc.)
    # Single quotes around $helper_script protect the path from word-splitting when the shell
    # evaluates the run-shell command.
    [[ -n "$toggle_key" ]] && pk_bind_shell "$toggle_key" "bash '$helper_script' toggle" "pomodoro:toggle"
    [[ -n "$start_key" ]] && pk_bind_shell "$start_key" "bash '$helper_script' start" "pomodoro:start"
    [[ -n "$stop_key" ]] && pk_bind_shell "$stop_key" "bash '$helper_script' stop" "pomodoro:stop"
    [[ -n "$skip_key" ]] && pk_bind_shell "$skip_key" "bash '$helper_script' skip" "pomodoro:skip"
}
